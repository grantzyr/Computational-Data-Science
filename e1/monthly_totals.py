...
def get_precip_data():
    return pd.read_csv('D:/Code/Python3/CMPT353/Ass1/e1/precipitation.csv', parse_dates=[2])
...
def pivot_months_pandas(data):
    ...
    grouped_data = data.groupby(['name', 'month']).aggregate('sum').reset_index()
    monthly = grouped_data.pivot(index='name', columns='month', values='precipitation')

    count = data.groupby(['name', 'month']).aggregate('count').reset_index()
    counts = count.pivot(index='name', columns='month', values='precipitation')
